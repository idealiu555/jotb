...
# Attention Mechanism Parameters (MASAC attention branches)
...
# 观测维度计算（统一包含 count 字段，MASAC attention 分支会用它们生成 mask）
# - 基础部分: uav_state + neighbors + ues
# - 额外字段: neighbor_count(1) + ue_count(1)
# OBS_DIM: 24+4*26+1+50*5+1 = 380
# MASAC AttentionEncoder 输出维度：UAV(128) + UE_attn(256) + Neighbor_attn(128) = 512
...
BASE_ACTOR_HIDDEN_DIM: int = 192  # actor width for MLP branches
BASE_CRITIC_HIDDEN_DIM: int = 384  # critic width for MLP branches
